...
export interface CombatSnapshot {
  ...
  archerRange?: { current: number; max: number; ampPerStack: number }; // 궁수 전용: 사거리 스택 (치명타 누적, 사망 시 0)
  clericLightseal?: { stacks: number; activeActionsLeft: number }; // 성직자 광휘의 인장 (스택 0~3, 효과 활성 잔여 행동)
  summonerFrenzy?: { activeMsLeft: number; cooldownMsLeft: number }; // 소환사 소환 폭주 — 활성 잔여 ms / 다음 발동까지 ms
