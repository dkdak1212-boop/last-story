  ...
  switch (best.effect_type) {
    ...
    case 'self_damage_pct': {
      ...
      if (d.miss) { log.push(`${me.name} [${best.name}] 빗나감`); }
        ...
        let dmg = d.damage;
        foe.hp -= dmg;
        log.push(`${me.name} [${best.name}] ${dmg}${d.crit ? '!' : ''}`);
    ...
    case 'lifesteal': {
      ...
      if (!d.miss) {
        foe.hp -= d.damage;
        const heal = Math.round(d.damage * best.effect_value / 100);
        me.hp = Math.min(me.maxHp, me.hp + heal);
        log.push(`${me.name} [${best.name}] ${d.damage}${d.crit ? '!' : ''} 흡혈+${heal}`);
    ...
    case 'multi_hit_poison': {
      ...
      for (let i = 0; i < hits; i++) {
        ...
        if (!d.miss) {
          foe.hp -= d.damage;
          log.push(`${me.name} [${best.name}] ${i + 1}타 ${d.damage}`);
    ...
    case 'dot': {
      ...
      if (!d.miss) {
        ...
        log.push(`${me.name} [${best.name}] ${d.damage} +도트${best.effect_duration}턴`);
      } else { log.push(`${me.name} [${best.name}] 빗나감`); }
      break;
    }
    case 'poison': {
      const d = calcDamage(me.stats, foe.stats, best.damage_mult, useMatk, best.flat_damage);
      if (!d.miss) foe.hp -= d.damage;
    ...
    case 'stun': {
      ...
      if (!d.miss) {
        ...
        log.push(`${me.name} [${best.name}] ${d.damage} 스턴!`);
      } else { log.push(`${me.name} [${best.name}] 빗나감`); }
      break;
    }
    case 'speed_mod':
    case 'self_speed_mod': {
      if (best.damage_mult > 0) {
        const d = calcDamage(me.stats, foe.stats, best.damage_mult, useMatk, best.flat_damage);
        if (!d.miss) { foe.hp -= d.damage; log.push(`${me.name} [${best.name}] ${d.damage}`); }
      }
    ...
    case 'shield_break': {
      ...
      const d = calcDamage(me.stats, foe.stats, best.damage_mult, useMatk, best.flat_damage);
      if (!d.miss) { foe.hp -= d.damage; log.push(`${me.name} [${best.name}] 실드파괴 ${d.damage}`); }
      break;
    }
    case 'heal_pct': {
