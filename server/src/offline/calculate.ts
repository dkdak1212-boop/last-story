...
import { isInCombat } from '../combat/engine.js';
...
export async function generateAndApplyOfflineReport(
  ...
  // 메모리상 활성 세션 존재 → 실시간 시뮬레이션 중이므로 중복 보상 방지
  // (서버 재시작 후 DB combat_sessions row 는 남아있을 수 있어 DB 체크 대신 in-memory 사용)
  if (!opts.dryRun && isInCombat(characterId)) {
    await query('UPDATE characters SET last_online_at = NOW() WHERE id = $1', [characterId]);
    return null;
  }
