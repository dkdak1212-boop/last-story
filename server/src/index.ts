...
async function runEquipOverhaul() {
  ...
  // 무한의 정수 — 영구 힘/덱스/인트/바이탈 보너스 + 모든 사냥터 1e-7 드랍 + 계정 일일 1개 제한
  {
    try {
      ...
      if (!applied.rowCount) {
        ...
        await query(`ALTER TABLE characters ADD COLUMN IF NOT EXISTS permanent_stat_bonus_dex INT NOT NULL DEFAULT 0`);
        await query(`ALTER TABLE characters ADD COLUMN IF NOT EXISTS permanent_stat_bonus_int INT NOT NULL DEFAULT 0`);
        await query(`ALTER TABLE characters ADD COLUMN IF NOT EXISTS permanent_stat_bonus_vit INT NOT NULL DEFAULT 0`);

        // 계정 일일 드랍 카운터 (KST 자정 리셋)
        await query(`ALTER TABLE users ADD COLUMN IF NOT EXISTS eternal_essence_drop_date DATE`);

        // 무한의 정수 아이템 (id 926)
        await query(
          `INSERT INTO items (id, name, type, grade, description, stack_size, sell_price, required_level)
           VALUES (926, '무한의 정수', 'consumable', 'mythic',
                   '사용 시 영구히 주력 스탯 (힘/덱스/인트/바이탈) 1개를 +1 합니다. 각 스탯 최대 +200까지. 계정당 하루 1개만 드랍됩니다.',
                   99, 0, 1)
           ON CONFLICT (id) DO NOTHING`
        );
  ...
  // 유니크 고정 옵션 통합 v2 — 옛날 굴림 무기 prefix_stats 에 unique 합산 추가
  // (이전: 옛날 굴림 무기는 prefix_stats 에 random 만 저장되어 unique 효과 미적용)
  // (이후: 모든 unique 무기 prefix_stats = unique + random 합산 형태로 통일)
  // 휴리스틱: prefix_stats[k] <= unique[k] 이면 옛날 (합산), 크면 신규 (이미 합산, skip)
  {
    try {
      const applied = await query(`SELECT 1 FROM _migrations WHERE name = 'unique_prefix_merge_v2'`);
      if (!applied.rowCount) {
        console.log('[마이그레이션] 유니크 고정옵션 통합 v2 — 옛날 굴림 prefix_stats 에 unique 추가...');
            ...
            UPDATE ${t.name} ${t.alias}
               SET prefix_stats = (
                 SELECT COALESCE(jsonb_object_agg(
                   k,
                   CASE
                     WHEN COALESCE((${t.alias}.prefix_stats->>k)::int, 0)
                          <= COALESCE((i.unique_prefix_stats->>k)::int, 0)
                       THEN COALESCE((${t.alias}.prefix_stats->>k)::int, 0)
                          + COALESCE((i.unique_prefix_stats->>k)::int, 0)
                     ELSE COALESCE((${t.alias}.prefix_stats->>k)::int, 0)
                   END
                 ), '{}'::jsonb)
                 FROM (
                   SELECT k FROM jsonb_object_keys(${t.alias}.prefix_stats) AS x(k)
                   UNION
                   SELECT k FROM jsonb_object_keys(i.unique_prefix_stats) AS y(k)
                 ) keys
               )
              FROM items i
        ...
        await query(`INSERT INTO _migrations (name) VALUES ('unique_prefix_merge_v2')`);
        console.log('[마이그레이션] 유니크 고정옵션 통합 v2 완료');
      }
    } catch (e) {
      console.error('[마이그레이션] 유니크 고정옵션 통합 v2 오류:', e);
    }
  }
  ...
  // 110제 시공 분쇄 무기 v2 — 도적 base str + 신규 유니크 옵션 (4옵 컨셉형)
  {
    try {
      const applied = await query(`SELECT 1 FROM _migrations WHERE name = 'lv110_weapon_rebalance_v2'`);
      if (!applied.rowCount) {
        await query(`UPDATE items SET unique_prefix_stats='{"atk_pct":25,"berserk_pct":30,"predator_pct":20,"def_pierce_pct":20}'::jsonb WHERE id=900`);
        await query(`UPDATE items SET unique_prefix_stats='{"matk_pct":25,"crit_dmg_pct":50,"gauge_on_crit_pct":15}'::jsonb WHERE id=901`);
      ...
      console.error('[late] lv110_weapon_rebalance_v2 error:', e);
    }
  }
