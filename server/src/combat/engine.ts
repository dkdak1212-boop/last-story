...
// HP 100% 가 가득 차서 손실되는 오버 회복도 그대로 누적 → 다음 자기 공격에 flat 합산.
// 발산 방지는 consumeHealStoredFlat 위치(applyDamagePrefixes 끝, multiplier 이후)로 보장됨 — cap 불필요.
// 안전망: playerMaxHp × 10 한도 (수치 오버플로 방지용 sanity cap, 정상 빌드에선 절대 도달 X).
function trackHealForKeystone(s: ActiveSession, intendedHeal: number): void {
  ...
  const sanityCap = s.playerMaxHp * 10;
  s.healStored = Math.min(sanityCap, (s.healStored || 0) + intendedHeal);
}
...
function enqueueStackDrop(d: PendingStackDrop): void {
  ...
  arr.push(d);
  // 2026-05-20: 진단 info 로그 제거 (운영 콘솔 스팸). 실패/리트라이/손실 로그만 유지.
}
  ...
  for (const d of drops) {
    ...
    if (nextRetry > STACK_DROP_MAX_RETRY) {
      if (isRiftMat(d.itemId)) {
        console.error(`[stack-drops] GIVE UP char=${d.characterId} item=${d.itemId} qty=${d.qty} retry=${d.retryCount}/${STACK_DROP_MAX_RETRY} reason=${reason}`);
      }
      continue;
    }
    enqueueStackDrop({ ...d, retryCount: nextRetry });
    // RETRY info 로그 제거 (DEADLOCK warn 에서 이미 표시됨)
  }
}
...
export async function flushPendingStackDrops(): Promise<void> {
  ...
  try {
    ...
    // Q1/Q2 진단 로그 제거 (2026-05-20). 실제 손실 발생 시 plan/DROP/DEADLOCK 로그가 표시됨.
    ...
    for (const characterId of allCharIds) {
      ...
      for (const [itemId, dInfo] of bucket.byItem) {
        ...
        while (remaining > 0) {
          ...
          if (slot === null) {
            // 가방 full — silent drop. 시공 재료는 손실 로그 유지 (실제 문제).
            if (traceRift) {
              console.warn(`[stack-drops] DROP char=${characterId} item=${itemId} lostQty=${remaining} reason=가방가득 maxSlots=${maxSlots}`);
            }
            break;
          }
      ...
      try {
        ...
        if (charUpdates.length > 0) {
          ...
          if (hasRiftDrops && (r.rowCount ?? 0) !== charUpdates.length) {
            console.warn(`[stack-drops] UPDATE mismatch char=${characterId} expected=${charUpdates.length} actual=${r.rowCount ?? 0}`);
          }
        }
        ...
        if (charInserts.length > 0) {
          ...
          if (riftExpected > 0 && insertRowCount !== charInserts.length) {
            console.warn(`[stack-drops] INSERT mismatch char=${characterId} expected=${charInserts.length} actual=${insertRowCount} (ON CONFLICT silent drop)`);
          }
        }
        ...
        if (isRetryableStackDropError(err)) {
          ...
          if (hasRiftDrops) {
            ...
            console.warn(`[stack-drops] DEADLOCK char=${characterId} code=${code} riftQty=${riftQty} → 재적재`);
          }
          ...
          if (riftDrops.length > 0) {
            console.error(`[stack-drops] FATAL char=${characterId} riftDrops=${riftDrops.length} (non-retryable)`, err);
          } else {
            console.error(`[stack-drops] non-retryable char=${characterId}`, err);
          }
        }
      }
    ...
    // 시공 재료가 큐에 있었으면 ALL drops 폐기됨 (snap.clear 이후 throw)
    ...
    if (riftPending.length > 0) {
      ...
      console.error(`[stack-drops] FLUSH FAILED — 시공 재료 ${riftPending.length}건 폐기 [${summary}]`, err);
    } else {
      console.error('[stack-drops] flush err', err);
    }
  }
}
...
export async function loadEquipPrefixes(characterId: number): Promise<Record<string, number>> {
  ...
  for (const row of r.rows) {
    ...
    const mult = 1 + el * 0.025; // 강화당 접두사 +2.5% (다른 경로와 통일)
...
function applyDexBuff(s: ActiveSession, dexAddPct: number, duration: number): void {
  ...
  // dodge cap — 클래스 기반: 도적/궁수 80%, 기타 70%.
  const dodgeCap = (s.className === 'rogue' || s.className === 'archer') ? 80 : 70;
...
function applyCritPostEffects(s: ActiveSession, dmg: number, crit: boolean, hitLabel: string = ''): void {
  ...
  if (critLifesteal > 0 && dmg > 0 && !s.monsterLifestealImmune) {
    ...
    if (critHeal > 0) {
      ...
      trackHealForKeystone(s, critHeal);
...
function processSummons(s: ActiveSession, extraDmgMul: number = 1.0) {
  ...
  // single_hit_amp_pct (묵직한/육중한/벽력의/천붕의 접두사) — 소환수에도 적용 (2026-05-30, 사용자 요청).
  const singleHitAmpForSummon = s.equipPrefixes.single_hit_amp_pct || 0;
  ...
  if (totalLifesteal > 0 && !s.monsterLifestealImmune) {
    ...
    trackHealForKeystone(s, totalLifesteal);
  }
  ...
  switch (skill.effect_type) {
    ...
    case 'summon_all': {
      ...
      if (ext > 0) {
        ...
        let extended = 0;
        for (const eff of s.statusEffects) {
          if (eff.type === 'summon' && eff.source === 'player' && eff.remainingActions > 0) {
            eff.remainingActions = Math.min(SUMMON_REMAIN_CAP, eff.remainingActions + ext);
            extended++;
          }
        }
        if (extended > 0) addLog(s, `[${skill.name}] 소환수 ${extended}마리 지속 +${ext}행동`);
      }
      break;
    }
...
// raid-bosses-v2: 발라카스 데미지 -65% 누적 (×0.7 × ×0.5 = ×0.35) (2026-05-17)
