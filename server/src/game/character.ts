...
export async function getEquippedItems(characterId: number) {
  ...
  return r.rows.map(row => {
    ...
    // 프리픽스 스탯 — 강화 배율 적용.
    // ce.prefix_stats = 굴림 random 만 (분리 저장). 유니크 고정은 effective 시점 합산.
...
export async function getEffectiveStats(char: CharacterRow): Promise<EffectiveStats> {
  ...
  // 키스톤 #15 빠른 결단 — 속도 +25% (페널티: 모든 데미지 −25%, engine.ts 에서 처리)
  if (pMap.has('paragon_quick_decision')) {
    eff.spd = Math.round(eff.spd * 1.25);
  }
  // 키스톤 #6 시간의 주인 — 속도 −25% (페널티: 모든 쿨다운 −25% 효과 대가, engine.ts 에서 cd 처리)
