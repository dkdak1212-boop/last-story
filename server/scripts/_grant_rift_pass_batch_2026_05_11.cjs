// 차원의 통행증(item 855) 일괄 지급 — 16명
// 한량: 3 + 2 두 번 호출 → 5장 합산 (동일 닉네임으로 추정)
...
(async () => {
  try {
    ...
    for (const [name, qty] of TARGETS) {
      ...
      if (!r.rowCount) { console.log(`[skip] NO CHAR ${name}`); failed.push(name); continue; }
      if (r.rowCount > 1) {
        console.log(`[skip] DUPLICATE name ${name} (${r.rowCount} rows) — 수동 확인 필요`);
        failed.push(name);
        continue;
      }
      ...
      if (stack.rowCount) {
        ...
        if (freeSlot < 0) { console.log(`[fail] ${name} 인벤토리 가득 참`); failed.push(name); continue; }
        await pool.query(
    ...
    console.log(`\n=== 완료: 총 ${totalGranted}장 지급 ===`);
    if (failed.length > 0) console.log(`실패 ${failed.length}명: ${failed.join(', ')}`);
  } finally {
    await pool.end();
  }
