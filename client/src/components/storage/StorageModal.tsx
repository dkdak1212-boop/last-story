...
interface StorageItem {
  ...
  quality: number;
  item: {
    id: number; name: string; grade: ItemGrade;
              ...
              {inventory.map(s => (
                <div key={s.slotIndex} style={{
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 11, fontWeight: 700, color: GRADE_COLOR[s.item.grade] || 'var(--text)' }}>
                      {s.item.name}
                      {s.enhanceLevel > 0 && <span style={{ marginLeft: 4, fontSize: 9, color: 'var(--accent)' }}>+{s.enhanceLevel}</span>}
                      {s.quantity > 1 && <span style={{ marginLeft: 4, fontSize: 9, color: 'var(--text-dim)' }}>×{s.quantity}</span>}
                    </div>
                  </div>
                  <button onClick={() => deposit(s.slotIndex)} disabled={busy} style={{ fontSize: 10, padding: '3px 8px' }}>
                    입고 →
                  </button>
                </div>
              ))}
            ...
            <div style={{ maxHeight: '50vh', overflow: 'auto', display: 'flex', flexDirection: 'column', gap: 4 }}>
              ...
              {data?.items.map(s => (
                <div key={s.id} style={{
                  ...
                  <button onClick={() => withdraw(s.id)} disabled={busy} style={{ fontSize: 10, padding: '3px 8px' }}>
                    ← 출고
                  </button>
                  <ItemIcon slot={s.item.slot ?? null} grade={s.item.grade} itemName={s.item.name} size={24} />
                  <div style={{ flex: 1, minWidth: 0 }}>
