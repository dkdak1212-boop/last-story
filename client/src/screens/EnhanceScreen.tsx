...
const STAT_KEY_LABEL: Record<string, string> = {
  ...
  ambush_pct: '각성', spd_pct: '속도증가', full_hp_amp_pct: '풀피증뎀', guardian_pct: '수호자',
  ...
  max_hp_pct: '최대HP%', all_stats_pct: '전체스탯%', drop_rate_pct: '드랍률', multi_hit_amp_pct: '다단뎀',
  def_pierce_pct: '방관', miss_combo_pct: '빗누적', evasion_burst_pct: '회피반격',
  shield_amp: '실드강화', summon_amp: '소환뎀', summon_double_hit: '소환2타',
  summon_max_extra: '소환+', summon_crit_dmg_amp: '소환크뎀', execute_pct: '처형', undispellable: '디스펠면역',
                ...
                <div key={`${it.kind}-${idx}`} onClick={() => { setSelected(it); setResult(null); setRerollIndex(null); }}
                  ...
                  {it.prefixStats && Object.keys(it.prefixStats).length > 0 && (
                    <div style={{ marginTop: 4 }}>
                      <PrefixDisplay prefixStats={it.prefixStats} prefixTiers={(it as any).prefixTiers} uniquePrefixStats={(it as any).uniquePrefixStats} enhanceLevel={(it as any).enhanceLevel} />
                    </div>
                  )}
                </div>
              );
                        ...
                        return (
                          <div key={key} style={{ fontSize: 11, padding: 6, background: 'rgba(255,255,255,0.03)', border: '1px dashed var(--border)', borderRadius: 3 }}>
                            ...
                            <div style={{ display: 'grid', gap: 2, fontFamily: 'monospace', fontSize: 10, color: 'var(--text-dim)' }}>
                              {uVal > 0 && (
                                <div>
                                  <span style={{ color: '#ff9933', textShadow: '0 1px 2px rgba(0,0,0,0.85), 0 0 3px rgba(0,0,0,0.6)' }}>┣ 유니크 고정</span>
